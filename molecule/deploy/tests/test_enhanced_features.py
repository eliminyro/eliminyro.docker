...
class TestEnhancedContainerFeatures:
    ...
    def test_postgres_container_configuration(self, docker_client):
        ...
        assert any('postgres' in tag for tag in image_tags), f"Expected postgres image, got: {image_tags}"
    ...
    def test_redis_container_configuration(self, docker_client):
        ...
        assert 'REDIS_APPENDONLY' in env_dict, "REDIS_APPENDONLY environment variable not set"
        assert env_dict['REDIS_APPENDONLY'] == 'yes', "REDIS_APPENDONLY has incorrect value"
        assert 'REDIS_APPENDFSYNC' in env_dict, "REDIS_APPENDFSYNC environment variable not set"
        assert env_dict['REDIS_APPENDFSYNC'] == 'everysec', "REDIS_APPENDFSYNC has incorrect value"
...
class TestContainerLabels:
    ...
    def test_main_container_labels(self, docker_client):
        ...
        assert labels['environment'] == 'test', f"Environment label incorrect: {labels['environment']}"
    ...
    def test_dependency_container_labels(self, docker_client):
        ...
        assert redis_labels['environment'] == 'test', f"Redis environment label incorrect: {redis_labels['environment']}"
        ...
        assert postgres_labels['app'] == 'postgres', f"PostgreSQL app label incorrect: {postgres_labels['app']}"
        assert 'environment' in postgres_labels, "PostgreSQL environment label not found"
        assert postgres_labels['environment'] == 'test', f"PostgreSQL environment label incorrect: {postgres_labels['environment']}"
...
class TestContainerEnvironmentVariables:
    ...
    def test_main_container_environment_variables(self, docker_client):
        ...
        assert env_dict['TEST_ENV_VAR'] == 'molecule-test-value', f"TEST_ENV_VAR has incorrect value: {env_dict['TEST_ENV_VAR']}"
        assert 'NGINX_ENVSUBST_TEMPLATE_SUFFIX' in env_dict, "NGINX_ENVSUBST_TEMPLATE_SUFFIX not set"
        assert env_dict['NGINX_ENVSUBST_TEMPLATE_SUFFIX'] == '.template', "NGINX_ENVSUBST_TEMPLATE_SUFFIX has incorrect value"
...
class TestVolumeManagement:
    ...
    def test_additional_volumes_mounted(self, docker_client):
        ...
        log_mount = next((m for m in mounts if m['Destination'] == '/var/log/nginx'), None)
