...
class TestVolumeToDirectoryMapping:
    ...
    def test_volume_path_parsing(self, host):
        ...
        for source, destination in expected_mappings.items():
            ...
            assert actual_mappings[source] == destination, f"Mount destination mismatch for {source}"
...
class TestConfigListProcessing:
    ...
    def test_config_file_deployment_from_list(self, host):
        ...
        mounts = container.attrs['Mounts']
        config_mounts = [m for m in mounts if m['Destination'] == '/etc/nginx/nginx.conf']
